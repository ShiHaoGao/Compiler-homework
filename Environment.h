            ...
            if (auto *vdecl = cl::dyn_cast<cl::VarDecl>(*i)) {
                ...
                if (cl::Expr *expr = vdecl->getInit()) {
                    if (auto *iLiteral = cl::dyn_cast<cl::IntegerLiteral>(expr)) {
                        val = (int)iLiteral->getValue().getSExtValue();
    ...
    void binop(cl::BinaryOperator *bop) {
        ...
        if (bop->isAssignmentOp()) {
            ...
            if (auto *declexpr = dyn_cast<cl::DeclRefExpr>(left)) {
                ...
                auto addr = mStack.back().getStmtVal(arrExpr->getBase());
                auto idx = mStack.back().getStmtVal(arrExpr->getIdx());
                auto ptr = addr + idx;
                if (isStackMem(ptr)) {
                    mStack.back().update(ptr, rVal);
                }
                else {
                    mHeap.Update(ptr, rVal);
                }
            } else if (auto *unop = dyn_cast<UnaryOperator>(left)) {
                // 得到地址，这里和上面一样，需要重新计算地址，因为*a在unaryOp里面计算的是值。
                auto ptrExpr = unop->getSubExpr();
                auto addr = mStack.back().getStmtVal(ptrExpr);
                auto ptr = addr;
                if (isStackMem(ptr)) {
                    mStack.back().update(addr, rVal);
                }
                else {
...
//        }
    ...
    void unOp(UnaryOperator *unop) {
        ...
        if (unOpc == UO_Minus) {
            ...
            // 解引用得到右值。
            if (isStackMem(rVal)) {
                result = mStack.back().get(rVal);
            }
            else {
                result = mHeap.get(rVal);
            }
        }
    ...
    bool isCondTrue(Expr *cond) {
        ...
        int condition = mStack.back().getStmtVal(cond);
        if (condition == 1)
            return true;
        else
            return false;
    ...
    void evalParenthesis(ParenExpr *parenExpr) {
        ...
        for (cl::DeclStmt::decl_iterator it = declstmt->decl_begin(), ie = declstmt->decl_end();
            ...
            if (auto *vardecl = dyn_cast<cl::VarDecl>(decl)) {
                ...
                if (type->isArrayType()) {
                    ...
                    int addr = mStack.back().allocStackMem(size);
...
//            if (mStack.back().hasDeclVal(decl)) {
//                addr = mStack.back().getDeclVal(decl);
//            } else {
...
//        }
    ...
    void arraySubscript(ArraySubscriptExpr *arrayExpr) {
        ...
        auto addr =mStack.back().getStmtVal(base);
        auto idx = mStack.back().getStmtVal(arrayExpr->getIdx());
        auto ptr = addr + idx;
        int result = 0;
        if (isStackMem(ptr)) {
            result = mStack.back().get(ptr);
        }
        else {
            result = mHeap.get(ptr);
        }
        mStack.back().bindStmt(arrayExpr, result);
    }

    bool isStackMem(int idx) {
        return idx < boundary;
    }
...
//            llvm::errs() << "Malloc!\n";
            ...
            // 得到指针
//            auto *declRef = callExpr->getArg(0);
//            auto addr = mStack.back().getStmtVal(declRef);
//            // 调用Free函数
//            mHeap.Free(ptr);
...
//            llvm::errs() << "Free!\n";
    ...
    Stmt* callReturn(ReturnStmt *returnStmt) {
